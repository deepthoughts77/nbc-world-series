...
const resolvePlayerId = async (rawId) => {
  ...
  // 1) Check players table first — most IDs come directly from players.id
  try {
  ...
  // If nothing matched, return original id
  return id;
};
...
export const getPlayerById = async (req, res) => {
  ...
  try {
    // 1) Resolve the REAL player_id
    const playerId = await resolvePlayerId(numericId);

    // 2) Verify player exists in players table
    ...
    // 3) Grab the player name
    const playerName = await getPlayerNameSafe(playerId);
    const nameParts = playerName.trim().split(" ");
    const firstName = nameParts[0] || "";
    const lastName = nameParts.slice(1).join(" ") || "";

    // 4) Batting stats across all years/teams
    const battingStatsResult = await pool.query(
      `SELECT 
        b.year, t.name AS team_name, t.city, t.state, t.id AS team_id,
    ...
    // 5) Pitching stats across all years/teams
    const pitchingStatsResult = await pool.query(
      `SELECT 
        p.year, t.name AS team_name, t.city, t.state, t.id AS team_id,
