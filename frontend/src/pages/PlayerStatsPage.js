...
export default function PlayerStatsPage() {
  ...
  const normalizedBattingRows = useMemo(() => {
    return (battingRows || []).map((r) => ({
      ...r,
      // keep whatever is present; navigation will resolve safely
  ...
  const nameMatches = useCallback((rowName, apiName) => {
  ...
  const resolveAndNavigateToPlayer = useCallback(
    async (row) => {
      ...
      const candidates = [
        ...
        row.playerId,
        row.player?.id,
        row.id, // may be wrong (stats row id), so we VERIFY before using it
      ].filter(Boolean);
